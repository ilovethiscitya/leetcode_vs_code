...
class Solution {
    public int calculateMinimumHP(int[][] d) {
        ...
        for(int i = m - 1; i >= 0; i--) {
            for(int j = n - 1; j >= 0; j--) {
                if(i == m - 1 && j == n - 1) {
                    results[j] = d[i][j]  >= 0 ? 0 : 1 - d[i][j];
                } else if(i == m - 1) {
                    results[j] = d[i][j] - results[j + 1] >= 0 ? 1 : results[j + 1] - d[i][j];
                } else if(j == n - 1){
                    results[j] = d[i][j] - results[j] >= 0 ? 1 : results[j] - d[i][j];
                }
                else {
                    int min = Math.max(results[j], results[j + 1]);
                    results[j] = d[i][j] - min >= 0 ? 1 : min - d[i][j];
                }
            }
        }
        return results[0];
